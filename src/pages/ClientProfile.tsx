...
export default function ClientProfile() {
  ...
  // Per-case pending fees — depend only on id (not the live cases array) to avoid infinite loop
  ...
  return (
    <div className="space-y-6 pb-8">
      {/* Back Button */}
      <button onClick={() => navigate('/clients')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition-colors">
        ...
        {editMode ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 md:col-span-1"><Label>Full Name *</Label><Input className="mt-1" value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} /></div>
              <div className="col-span-2 md:col-span-1"><Label>Mobile *</Label><Input className="mt-1" value={editForm.mobile} onChange={e => setEditForm(f => ({ ...f, mobile: e.target.value }))} /></div>
              <div className="col-span-2 md:col-span-1"><Label>Email</Label><Input className="mt-1" type="email" value={editForm.email} onChange={e => setEditForm(f => ({ ...f, email: e.target.value }))} /></div>
              <div className="col-span-2 md:col-span-1"><Label>Address</Label><Input className="mt-1" value={editForm.address} onChange={e => setEditForm(f => ({ ...f, address: e.target.value }))} /></div>
            ...
            <div className="flex gap-2">
              <Button onClick={saveEdit} disabled={editLoading} className="bg-blue-600 hover:bg-blue-700"><Save className="w-4 h-4 mr-1.5" />{editLoading ? 'Saving...' : 'Save Changes'}</Button>
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200" onClick={() => setShowDelete(true)}>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-900">Cases ({cases.length})</h2>
          </div>
          <Link to="/clients" className="text-xs text-blue-600 hover:text-blue-700 font-medium hidden">View all</Link>
        </div>
        {cases.length === 0 ? (
                ...
                <div key={c.id} onClick={() => navigate(`/clients/${clientId}/cases/${c.id}`)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">{c.title}</span>
                      {c.caseNumber && <span className="text-xs text-gray-400 font-mono">#{c.caseNumber}</span>}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-900">Communication History ({communications.length})</h2>
          </div>
            ...
            <form onSubmit={submitComm} className="space-y-3">
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Input className="mt-1 h-9 bg-white text-sm" type="date" value={commForm.date} onChange={e => setCommForm(f => ({ ...f, date: e.target.value }))} />
                </div>
              </div>
              <div>
              ...
              <div className="flex gap-2">
                <Button type="submit" size="sm" disabled={commLoading} className="bg-blue-600 hover:bg-blue-700">{commLoading ? 'Saving...' : 'Log Communication'}</Button>
                <Button type="button" size="sm" variant="outline" onClick={() => setShowAddComm(false)}>Cancel</Button>
              </div>
            </form>
          </div>
        )}
        ...
        {communications.length === 0 && !showAddComm ? (
          ...
          <div className="divide-y divide-gray-50">
            {communications.map(comm => (
              <div key={comm.id} className="flex items-start gap-4 px-5 py-4">
