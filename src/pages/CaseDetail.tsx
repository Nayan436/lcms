  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <form onSubmit={submitNote} className="bg-blue-50 rounded-xl border border-blue-100 p-4">
        ...
        />
        <Button type="submit" size="sm" disabled={loading || !noteContent.trim()} className="bg-blue-600 hover:bg-blue-700 min-h-[44px]">
            ...
            <div className="space-y-3">
              {grouped[dateKey].map((entry, i) => (
                <div key={entry.id ?? i} className="flex items-start gap-3">
                  <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-gray-100 border-2 border-white shadow-sm flex items-center justify-center flex-shrink-0 text-base mt-0.5">
                    {getTimelineIcon(entry.type)}
                  </div>
                  <div className="flex-1 bg-white rounded-xl border border-gray-100 shadow-sm p-3 md:p-4">
      ...
      <div className={`p-4 rounded-xl border ${highlight ? 'border-blue-200 bg-blue-50' : 'border-gray-100 bg-white'} shadow-sm`}>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="flex flex-wrap gap-2 md:gap-3 text-sm text-gray-600">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setEditStatus(s => !s)} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center">
          ...
          <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap items-center gap-2">
            ...
            {HEARING_STATUSES.map(s => (
              <button key={s} onClick={async () => { if (h.id) { await updateHearing(h.id, { status: s as any }); setEditStatus(false); toast({ title: 'Status updated' }) } }}
                className={`text-xs px-2.5 py-1.5 rounded-full border font-medium transition-colors min-h-[36px] ${h.status === s ? getHearingStatusColor(s) + ' border-current' : 'text-gray-500 border-gray-200 hover:bg-gray-50'}`}>
                {s}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <div className="flex justify-end">
        <Button onClick={() => setShowAdd(s => !s)} variant="outline" size="sm" className="min-h-[44px]">
      ...
      {showAdd && (
        <form onSubmit={submitHearing} className="bg-green-50 rounded-xl border border-green-100 p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-xs">Date *</Label><Input className="mt-1 h-10 bg-white text-sm" type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} required /></div>
            <div><Label className="text-xs">Time *</Label><Input className="mt-1 h-10 bg-white text-sm" type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))} required /></div>
            <div><Label className="text-xs">Court Number</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="Court No. 5" value={form.courtNumber} onChange={e => setForm(f => ({ ...f, courtNumber: e.target.value }))} /></div>
            <div><Label className="text-xs">Judge Name</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="Hon. Justice..." value={form.judgeName} onChange={e => setForm(f => ({ ...f, judgeName: e.target.value }))} /></div>
            <div className="col-span-2"><Label className="text-xs">Remarks</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="Optional remarks" value={form.remarks} onChange={e => setForm(f => ({ ...f, remarks: e.target.value }))} /></div>
          </div>
          <div className="flex gap-2">
            <Button type="submit" size="sm" disabled={loading || !form.date} className="bg-green-600 hover:bg-green-700 min-h-[44px]">{loading ? 'Adding...' : 'Schedule Hearing'}</Button>
            <Button type="button" size="sm" variant="outline" onClick={() => setShowAdd(false)} className="min-h-[44px]">Cancel</Button>
          </div>
        </form>
      )}
...
function DocumentsTab({ caseId, clientId }: { caseId: number; clientId: number }) {
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <div className="flex justify-end">
        <Button onClick={() => setShowAdd(s => !s)} variant="outline" size="sm" className="min-h-[44px]"><Plus className="w-4 h-4 mr-1.5" />Add Document</Button>
      </div>
      ...
      {showAdd && (
        <form onSubmit={submitDocument} className="bg-purple-50 rounded-xl border border-purple-100 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><Label className="text-xs">Document Name *</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="e.g. FIR Copy" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required /></div>
          ...
          <div className="flex gap-2">
            <Button type="submit" size="sm" disabled={loading || !form.name.trim()} className="bg-purple-600 hover:bg-purple-700 min-h-[44px]">{loading ? 'Adding...' : 'Add Document'}</Button>
            <Button type="button" size="sm" variant="outline" onClick={() => setShowAdd(false)} className="min-h-[44px]">Cancel</Button>
          </div>
        </form>
      )}
          ...
          {[...DOCUMENT_CATEGORIES, ...Array.from(new Set(documents.map(d => d.category).filter(c => !DOCUMENT_CATEGORIES.includes(c))))].map(cat => {
            ...
            return (
              <div key={cat}>
                ...
                {catDocs.map(doc => (
                  <div key={doc.id} className={`flex items-center gap-3 px-4 py-3.5 transition-colors hover:bg-gray-50/50 ${doc.checked ? 'bg-green-50/30' : ''}`}>
                    <Checkbox
                      id={`doc-${doc.id}`}
                      checked={doc.checked}
                    ...
                    <button onClick={async () => { if (doc.id) { await deleteDocument(doc.id); toast({ title: 'Document removed' }) } }}
                      className="p-1.5 rounded hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors flex-shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center">
    ...
    <div className="space-y-4 md:space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          ...
          <div key={label} className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 text-center">
            ...
            <p className={`text-base md:text-lg ${cls}`}>{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          ...
          <Button variant="outline" size="sm" onClick={() => setShowAddCharge(s => !s)} className="min-h-[44px]"><Plus className="w-3.5 h-3.5 mr-1" />Add Charge</Button>
        </div>
        {showAddCharge && (
          <form onSubmit={submitCharge} className="px-4 py-3 bg-orange-50 border-b border-orange-100 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 sm:col-span-1"><Label className="text-xs">Description *</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="e.g. Stamp Duty" value={chargeForm.label} onChange={e => setChargeForm(f => ({ ...f, label: e.target.value }))} required /></div>
              <div><Label className="text-xs">Amount (₹) *</Label><Input className="mt-1 h-10 bg-white text-sm" type="number" min="1" placeholder="0" value={chargeForm.amount} onChange={e => setChargeForm(f => ({ ...f, amount: e.target.value }))} required /></div>
            </div>
            <div className="flex gap-2">
              <Button type="submit" size="sm" disabled={chargeLoading} className="bg-orange-600 hover:bg-orange-700 min-h-[44px]">{chargeLoading ? 'Adding...' : 'Add Charge'}</Button>
              <Button type="button" size="sm" variant="outline" onClick={() => setShowAddCharge(false)} className="min-h-[44px]">Cancel</Button>
            </div>
          </form>
        )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          ...
          <Button variant="outline" size="sm" onClick={() => setShowAddPayment(s => !s)} className="min-h-[44px]"><Plus className="w-3.5 h-3.5 mr-1" />Add Payment</Button>
        </div>
        {showAddPayment && (
          <form onSubmit={submitPayment} className="px-4 py-3 bg-green-50 border-b border-green-100 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">Amount (₹) *</Label><Input className="mt-1 h-10 bg-white text-sm" type="number" min="1" placeholder="5000" value={paymentForm.amount} onChange={e => setPaymentForm(f => ({ ...f, amount: e.target.value }))} required /></div>
              <div><Label className="text-xs">Date *</Label><Input className="mt-1 h-10 bg-white text-sm" type="date" value={paymentForm.date} onChange={e => setPaymentForm(f => ({ ...f, date: e.target.value }))} required /></div>
              <div className="col-span-2 sm:col-span-1"><Label className="text-xs">Mode</Label>
                <Select value={paymentForm.paymentMode} onValueChange={(v: any) => setPaymentForm(f => ({ ...f, paymentMode: v }))}>
                  <SelectTrigger className="mt-1 h-10 bg-white text-sm"><SelectValue /></SelectTrigger>
                  <SelectContent>{PAYMENT_MODES.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="col-span-2"><Label className="text-xs">Remarks</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="Optional remarks" value={paymentForm.remarks} onChange={e => setPaymentForm(f => ({ ...f, remarks: e.target.value }))} /></div>
            </div>
            <div className="flex gap-2">
              <Button type="submit" size="sm" disabled={payLoading} className="bg-green-600 hover:bg-green-700 min-h-[44px]">{payLoading ? 'Recording...' : 'Record Payment'}</Button>
              <Button type="button" size="sm" variant="outline" onClick={() => setShowAddPayment(false)} className="min-h-[44px]">Cancel</Button>
            </div>
          </form>
        )}
        {payments.length === 0 ? (
...
function TasksTab({ caseId, clientId }: { caseId: number; clientId: number }) {
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <div className="flex justify-end">
        <Button onClick={() => setShowAdd(s => !s)} variant="outline" size="sm" className="min-h-[44px]"><Plus className="w-4 h-4 mr-1.5" />Add Task</Button>
      </div>
      ...
      {showAdd && (
        <form onSubmit={submitTask} className="bg-amber-50 rounded-xl border border-amber-100 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><Label className="text-xs">Task Title *</Label><Input className="mt-1 h-10 bg-white text-sm" placeholder="e.g. File bail application" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required /></div>
            ...
            <div><Label className="text-xs">Due Date</Label><Input className="mt-1 h-10 bg-white text-sm" type="date" value={form.dueDate} onChange={e => setForm(f => ({ ...f, dueDate: e.target.value }))} /></div>
          ...
          <div className="flex gap-2">
            <Button type="submit" size="sm" disabled={loading} className="bg-amber-600 hover:bg-amber-700 min-h-[44px]">{loading ? 'Adding...' : 'Add Task'}</Button>
            <Button type="button" size="sm" variant="outline" onClick={() => setShowAdd(false)} className="min-h-[44px]">Cancel</Button>
          </div>
        </form>
      )}
              ...
              {pending.map(task => (
                <div key={task.id} className="flex items-center gap-3 px-4 py-3.5 group">
                  <button onClick={async () => { if (task.id) { await updateTask(task.id, { status: 'Completed' }); toast({ title: 'Task completed' }) } }}
                    className="w-6 h-6 rounded-full border-2 border-gray-300 hover:border-green-500 flex items-center justify-center flex-shrink-0 transition-colors">
                  ...
                  <button onClick={async () => { if (task.id) { await deleteTask(task.id); toast({ title: 'Task deleted' }) } }}
                    className="opacity-0 group-hover:opacity-100 p-1.5 rounded hover:bg-red-50 text-gray-300 hover:text-red-500 transition-all flex-shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              ))}
...
function CommunicationTab({ caseId, clientId }: { caseId: number; clientId: number }) {
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <div className="flex justify-end">
        <Button onClick={() => setShowAdd(s => !s)} variant="outline" size="sm" className="min-h-[44px]"><Plus className="w-4 h-4 mr-1.5" />Log Communication</Button>
      </div>
      ...
      {showAdd && (
        <form onSubmit={submitComm} className="bg-blue-50 rounded-xl border border-blue-100 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label className="text-xs">Date</Label><Input className="mt-1 h-10 bg-white text-sm" type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} /></div>
          ...
          <div className="flex gap-2">
            <Button type="submit" size="sm" disabled={loading} className="bg-blue-600 hover:bg-blue-700 min-h-[44px]">{loading ? 'Saving...' : 'Log Communication'}</Button>
            <Button type="button" size="sm" variant="outline" onClick={() => setShowAdd(false)} className="min-h-[44px]">Cancel</Button>
          </div>
        </form>
      )}
      ...
      {caseComms.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm divide-y divide-gray-50">
          {caseComms.map(comm => (
            <div key={comm.id} className="flex items-start gap-3 md:gap-4 px-4 py-4">
...
export default function CaseDetail() {
  ...
  return (
    <div className="space-y-4 md:space-y-6 pb-4">
      {/* Back — touch-friendly */}
      <button onClick={() => navigate(`/clients/${clientId}`)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition-colors min-h-[44px]">
        ...
        {editMode ? (
          <div className="space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <Button onClick={saveEdit} disabled={editLoading} className="bg-blue-600 hover:bg-blue-700 min-h-[44px]"><Save className="w-4 h-4 mr-1.5" />{editLoading ? 'Saving...' : 'Save'}</Button>
              <Button variant="outline" onClick={() => setEditMode(false)} className="min-h-[44px]"><X className="w-4 h-4 mr-1.5" />Cancel</Button>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex items-start justify-between flex-wrap gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 md:gap-3 flex-wrap mb-2">
                  <h1 className="text-lg md:text-xl font-bold text-gray-900">{caseData.title}</h1>
                  {caseData.caseNumber && <span className="text-xs text-gray-400 font-mono bg-gray-100 px-2 py-0.5 rounded hidden sm:inline">#{caseData.caseNumber}</span>}
                </div>
                <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600">
                  ...
                  <span className="hidden sm:inline">{caseData.courtName}</span>
                  <span className="hidden sm:inline">·</span>
                  <span className="hidden sm:inline">{caseData.caseType}</span>
                  {caseData.oppositeParty && <><span className="hidden sm:inline">·</span><span className="hidden sm:inline">vs {caseData.oppositeParty}</span></>}
                </div>
                {/* On mobile, show court/type on separate line */}
                <div className="flex flex-wrap items-center gap-1.5 text-xs text-gray-500 mt-1 sm:hidden">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <Select value={caseData.status} onValueChange={changeStatus} disabled={statusChanging}>
                  <SelectTrigger className={`h-9 text-xs font-semibold w-auto px-2 border ${getCaseStatusColor(caseData.status)}`}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>{CASE_STATUSES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
            ...
            {/* Financial Summary Bar — horizontal scroll on mobile */}
            <div className="flex gap-3 md:gap-4 pt-3 md:pt-4 border-t border-gray-100 overflow-x-auto pb-1">
              {[
                ...
                <div key={label} className="text-center flex-shrink-0">
                  ...
                  <p className={`text-sm md:text-base ${cls}`}>{val}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <Tabs defaultValue="timeline">
        <div className="overflow-x-auto -mx-4 px-4">
          <TabsList className="bg-white border border-gray-200 p-1 rounded-xl shadow-sm w-max min-w-full justify-start">
            <TabsTrigger value="timeline" className="rounded-lg text-sm whitespace-nowrap">Timeline</TabsTrigger>
            <TabsTrigger value="hearings" className="rounded-lg text-sm whitespace-nowrap">Hearings</TabsTrigger>
            <TabsTrigger value="documents" className="rounded-lg text-sm whitespace-nowrap">Documents</TabsTrigger>
            <TabsTrigger value="payments" className="rounded-lg text-sm whitespace-nowrap">Payments</TabsTrigger>
            <TabsTrigger value="tasks" className="rounded-lg text-sm whitespace-nowrap">Tasks</TabsTrigger>
            <TabsTrigger value="communication" className="rounded-lg text-sm whitespace-nowrap">Comms</TabsTrigger>
          </TabsList>
        </div>

        <TabsContent value="timeline" className="mt-4 md:mt-5">
          <TimelineTab caseId={caseData.id!} clientId={caseData.clientId} />
        </TabsContent>
        <TabsContent value="hearings" className="mt-4 md:mt-5">
          <HearingsTab caseId={caseData.id!} clientId={caseData.clientId} />
        </TabsContent>
        <TabsContent value="documents" className="mt-4 md:mt-5">
          <DocumentsTab caseId={caseData.id!} clientId={caseData.clientId} />
        </TabsContent>
        <TabsContent value="payments" className="mt-4 md:mt-5">
          <PaymentsTab caseId={caseData.id!} clientId={caseData.clientId} fixedFee={caseData.fixedFee} />
        </TabsContent>
        <TabsContent value="tasks" className="mt-4 md:mt-5">
          <TasksTab caseId={caseData.id!} clientId={caseData.clientId} />
        </TabsContent>
        <TabsContent value="communication" className="mt-4 md:mt-5">
